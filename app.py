...
def _parse_pages_cached(page_str: str, total_pages: int):
    """
    Parse page spec string into a tuple of 0-based page indices.
    Supported formats: "all", "1", "1-3", "1,3,5"
    """
...
def filter_occurrence(page_indices: list, occurrence: str) -> list:
    """Filter page indices by occurrence: all, first, or last."""
    if not page_indices:
        return page_indices
...
def _resolve_stamp_size(
    ...
    """
    Resolve final stamp dimensions in PDF points.

    Priority:
      1. stamp_width_percent / stamp_height_percent
         (% of page width / height — sent from sheet I12/J12)
      2. stamp_width_px / stamp_height_px (fixed point values)
      3. Default fallback: 15% x 10% of page

    The sheet sends a SINGLE shared value for all rows
    (e.g. 2 means 2% of the page dimension).
    """
    if stamp_width_percent is not None and stamp_height_percent is not None:
        ...
        if sw <= 0 or sh <= 0:
            raise ValueError(
                "Stamp size resolved to zero or negative: "
                "sw={} sh={} (w%={} h%={})".format(
    ...
    if stamp_width_px is not None and stamp_height_px is not None:
        ...
        logger.info(
            "✅ Stamp size (fixed px/pt) → sw=%.2fpt sh=%.2fpt", sw, sh
        )
        return sw, sh
    ...
    sh = 0.10 * page_h
    logger.warning(
        "⚠️  Stamp size not specified — using default 15%%x10%%: "
...
def build_stamp_overlay(
    ...
    """
    Build a single-page PDF overlay containing the stamp image
    and optional timestamp text.

    Coordinate system:
      - ReportLab origin is BOTTOM-LEFT.
      - Input x/y percents measure from TOP-LEFT (CSS-style).
      - flip_x / flip_y can correct axis orientation if needed.
    """
    packet = io.BytesIO()
    ...
    if date_text:
        ...
        # Position text just above the stamp area (+ 2pt padding)
...
def stamp_pdf(
    ...
    """
    Apply the stamp image to the specified pages of the PDF.
    ...
    logger.info(
        "📄 PDF pages: %d total | stamping pages: %s",
        total,
...
def health():
    """Health check endpoint — used by the Apps Script to wake the server."""
...
def stamp_endpoint():
    ...
    if content_length and content_length > MAX_BODY_BYTES:
        logger.warning(
            "🚫 Request too large: %d bytes (max %d)", content_length, MAX_BODY_BYTES
        )
    ...
    if not pdf_b64:
        return jsonify({"error": "Missing required field: pdf (base64)"}), 400
    if not stamp_b64:
        return jsonify({"error": "Missing required field: stamp (base64)"}), 400
    if x_pct is None:
    ...
    if not (0 <= x_pct <= 100):
        return jsonify({"error": "x_percent must be between 0 and 100"}), 400
    if not (0 <= y_pct <= 100):
        return jsonify({"error": "y_percent must be between 0 and 100"}), 400

    # ── Optional fields ────────────────────────────────────
    # stamp_width_percent / stamp_height_percent:
    #   Sent from the shared I12/J12 sheet values (e.g. 2 = 2% of page).
    #   The same value is used for every stamped row.
    ...
    # ── Validate optional stamp size ───────────────────────
    if stamp_width_percent is not None and stamp_width_percent <= 0:
        return jsonify({"error": "stamp_width_percent must be > 0"}), 400
    if stamp_height_percent is not None and stamp_height_percent <= 0:
        return jsonify({"error": "stamp_height_percent must be > 0"}), 400
    if stamp_width_percent is not None and stamp_width_percent > 100:
        return jsonify({"error": "stamp_width_percent must be <= 100"}), 400
    if stamp_height_percent is not None and stamp_height_percent > 100:
        return jsonify({"error": "stamp_height_percent must be <= 100"}), 400
    ...
    logger.info("  stamp_width_percent  = %s  ← from sheet I12 (shared)", stamp_width_percent)
    logger.info("  stamp_height_percent = %s  ← from sheet J12 (shared)", stamp_height_percent)
    ...
    logger.info(
        "📦 PDF size: %d bytes | Stamp size: %d bytes",
    ...
    # Return both keys for backward compatibility with the Apps Script
    return jsonify({
        "pdf":         result_b64,
